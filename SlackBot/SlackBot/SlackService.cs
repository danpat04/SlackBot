...
namespace SlackBot
{
    ...
    public class SlackService : ISlackService
    {
        ...
        public async Task<Message> PostMessageAsync(string workspaceName, string channelId, string text)
        {
            ...
            if (!response.Ok)
            {
                _logger.LogInformation($"Tried to send message to {workspaceName}/{channelId} : {text}");
        ...
        public async Task PostEphemeralAsync(string workspaceName, string channelId, string userId, string text)
        {
            ...
            if (!response.Ok)
            {
                _logger.LogInformation($"Tried to send message to {workspaceName}/{channelId}/{userId} : {text}");
                _logger.LogError($"Sending failed: {response.Error}");
            }
        }
    }
